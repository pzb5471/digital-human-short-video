...
5. Copy current prices into `DHSV_ALIYUN_CNY_PER_MINUTE` and `DHSV_COSYVOICE_CNY_PER_1000_CHARACTERS`, then rerun `plan`; do not treat documentation examples as live quotes.
...
4. Prefer `HEYGEN_AVATAR_ID` or a cached avatar. Set `HEYGEN_IMAGE_FALLBACK=true` only after an explicit decision to use direct image mode; empty or `false` keeps fallback disabled.
