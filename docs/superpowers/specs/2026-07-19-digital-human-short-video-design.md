...
选择理由：支持自定义照片数字人、9:16 与 16:9、文本或音频脚本以及可选水印；国内网络、中文语音和人民币结算更适合默认场景。价格和套餐属于易变信息，每次执行前查询官方计费页并写入当前估算费率。
...
HeyGen 价格和最低消费属于易变信息，每次执行前必须查询官方计费页并写入当前估算费率。适配器必须确认当前账户输出不带平台水印，且不得代表用户充值、订阅或升级。
...
1. 数字人 API 阶段：验证输入与授权，生成口播稿和字幕节拍，估算费用，使用已配置或已缓存的数字人（或显式批准的直接图片模式），提交异步任务并下载数字人口播原片。
...
初稿按约 3.5 个汉字/秒估算长度。该数值只用于控制初稿，最终时长必须以 TTS 生成后的真实音频为准。脚本不得编造数据、价格、承诺或案例；缺乏证据时使用定性表达。生成脚本后先让用户分别确认脚本 SHA-256 与序列化估算 SHA-256，再进行付费配音；旁白完成后再用绑定当前旁白哈希的独立审批提交数字人付费调用。

### 8.2 分段配音

默认使用兼容公共音色的阿里云百炼 `cosyvoice-v3-flash` 与 `longanyang`。每个脚本段落单独生成音频并请求词级时间戳，然后按 `pause_after_ms` 添加停顿并合并为 `narration.wav`。
...
CosyVoice HTTP API 可返回词或字符的起止毫秒时间。价格属于易变信息，执行时必须查询当前官方价格、写入 `DHSV_COSYVOICE_CNY_PER_1000_CHARACTERS`，并用 `plan` 生成新的精确估算审批。
...
8. 使用已配置或已缓存的照片数字人（阿里云在缺少映射时可按官方字段创建；HeyGen 缺少 avatar 时仅允许显式直接图片模式），上传最终音频并提交异步任务，将服务商、任务 ID、时间、预计费用、音频校验值和状态写入 `state.json`。
...
- 认证失败、余额不足和审核拒绝直接报告。
- 只有安全的 GET/HEAD 查询在 429 和临时 5xx 时使用有界退避。
- 不自动重试任何可能收费的 TTS 或数字人 POST；含糊结果必须通过持久化意图和原任务 ID 人工恢复。
